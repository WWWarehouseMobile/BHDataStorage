...
@implementation BHKeychainQuery
...
- (BOOL)save:(NSError *__autoreleasing *)error {
	...
	if (status == errSecSuccess) {//item already exists, update it!
		...
		CFTypeRef accessibilityType = [BHKeychain accessibilityType];
...
+ (NSError *)errorWithCode:(OSStatus) code {
	...
	dispatch_once(&onceToken, ^{
		NSURL *url = [[NSBundle bundleForClass:[BHKeychainQuery class]] URLForResource:@"SAMKeychain" withExtension:@"bundle"];
	...
	return [NSError errorWithDomain:kBHKeychainErrorDomain code:code userInfo:userInfo];
}
